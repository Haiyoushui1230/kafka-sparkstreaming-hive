...
public class KafkaOffsetTool {
  ...
  /**
   * Map<TopicAndPartition, Long> ，Long 是该TopicAndPartition的偏移量offset
   * @param brokerList
   * @param topics
   * @param groupId
   * @return
  ...
  public Map<TopicAndPartition, Long> getLastOffset(String brokerList, List<String> topics,
    ...
    Map<TopicAndPartition, BrokerEndPoint> topicAndPartitionBrokerMap =
        KafkaOffsetTool.getInstance().findLeader(brokerList, topics);//得到所有的 (topicAndPartition, part.leader())
  ...
  /**
   * 得到broker url 与 其port 的映射关系,map(ip,port)
   *
   * @param brokerlist
   * @return
   */
